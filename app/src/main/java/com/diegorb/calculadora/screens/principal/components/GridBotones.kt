    ...
    Row(
        ...
        valor.map { tipoBoton ->
            BotonCalculadora(
                ...
                color = if (tipoBoton.getTextColor() != null)tipoBoton.getTextColor()!! else MaterialTheme.colorScheme.tertiary
                ,
                icon = tipoBoton.icon,
                roundness = 12,
...
val rows = listOf(
    listOf(
        ...
        BotonTipo("±", ColorBoton.COLOR_ACCION, action = Accion.CambiaSigno),
        BotonTipo("%", ColorBoton.COLOR_ACCION, action = Accion.PorCiento),
    ...
    listOf(
        ...
        BotonTipo("×", ColorBoton.COLOR_OPERACION, action = Accion.Operar(Operacion.Multiplicar))
    ),
    listOf(
    ...
    listOf(
        BotonTipo(
            color = ColorBoton.COLOR_ESPECIAL,
            icon = com.diegorb.calculadora.R.drawable.delete_icon,
...
private fun BotonTipo.getTextColor(): Color? {
    return when (color) {
        ColorBoton.COLOR_NUMEROS -> {
            null
        }
        ColorBoton.COLOR_ACCION -> {
            Verde
        }
        ColorBoton.COLOR_OPERACION -> {
            Rojo
        }
        ColorBoton.COLOR_ESPECIAL -> {
            Naranja
        }
    }
}